...
func RoleCommand(ctx framework.Context) {
    ...
    if len(ctx.Args) == 0{
        ctx.Reply("Choose a role to add : htb, paris, rennes, toulouse, tours, lille, aix-marseille, canada")
        return
    }
    ...
    if ! framework.IsInSlice(roleInput, []string{"htb", "paris", "rennes", "toulouse", "tours", "lille", "aix-marseille", "canada"}){
